...
public class Cooking extends JPanel {
    ...
    private Image     cutFishImage;
    private Image     cookedFishImage;
    private boolean   showCooked = false;
    private long      cookedEndTime = 0;
    ...
    public Cooking() {
        ...
        arrowRight.setBounds(1266, 400, 100, 100);
    ...
    public static void startMinigame(String fishName) {
        ...
        instance.health       = 0.25f;
        instance.progress     = 0.25f;
        instance.circlesDone  = 0;
        instance.circleActive = false;
        instance.minigame     = true;
        instance.showCooked   = false;
        instance.loadFishImages(fishName);
        isCooking             = true;
    ...
    if (success) {
            ...
            showCooked = true;
            cookedEndTime = System.currentTimeMillis() + 1000;
            new Timer(1000, ev -> { showCooked = false; repaint(); ((Timer) ev.getSource()).stop(); }).start();
        } else {
...
private void loadImages() {
    ...
    private void loadFishImages(String fishName) {
        // fishName is like "Cut Bass" — strip "Cut " to get base name
        String base = fishName.replace("Cut ", "").toLowerCase();
        try {
            cutFishImage   = new ImageIcon(getClass().getResource("/images/cut"   + base.substring(0,1).toUpperCase() + base.substring(1) + "Grill.png")).getImage();
            cookedFishImage = new ImageIcon(getClass().getResource("/images/cooked" + base.substring(0,1).toUpperCase() + base.substring(1) + "Grill.png")).getImage();
        } catch (Exception e) {
            cutFishImage    = null;
            cookedFishImage = null;
        }
    }
    ...
    class BackgroundPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (!minigame && !showCooked) {
                        ...
                        if (minigame && cutFishImage != null)
                            g2.drawImage(cutFishImage, 0, 0, getWidth(), getHeight(), this);
                        if (showCooked && cookedFishImage != null)
                            g2.drawImage(cookedFishImage, 0, 0, getWidth(), getHeight(), this);
