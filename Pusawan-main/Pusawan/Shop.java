...
public class Shop extends JFrame {
    ...
    public static final java.util.Map<String, Integer> buyPrices = new java.util.LinkedHashMap<>();
    public static final java.util.Map<String, Integer> buyItems = new java.util.LinkedHashMap<>();

    private boolean isBuyItemsTab = false;

    static {
    // Cooked Fish (highest value tier)
    sellPrices.put("Cooked Bass",    155);
    sellPrices.put("Cooked Catfish", 115);
    sellPrices.put("Cooked Carp",     80);
    sellPrices.put("Cooked Perch",    50);

    // Raw Fish
    sellPrices.put("Bass",    55);
    sellPrices.put("Catfish", 40);
    sellPrices.put("Carp",    28);
    sellPrices.put("Perch",   16);

    // Cut Fish (processed tier)
    sellPrices.put("Cut Bass",    80);
    sellPrices.put("Cut Catfish", 60);
    sellPrices.put("Cut Carp",    42);
    sellPrices.put("Cut Perch",   26);

    // Bait (resale value only / junk economy)
    sellPrices.put("Worm Bait",    2);
    sellPrices.put("Insect Bait",  6);
    sellPrices.put("Fish Bait",   18);
    sellPrices.put("Magic Bait",  55);

    // Trash items
    sellPrices.put("Sandal",          1);
    sellPrices.put("Shoe",            1);
    sellPrices.put("Plastic Wrapper", 0);


    // ================= BUY PRICES =================

    buyPrices.put("Worm Bait",   8);
    buyPrices.put("Insect Bait", 22);
    buyPrices.put("Fish Bait",   60);
    buyPrices.put("Magic Bait", 160);

    buyItems.put("Default Rod", 100);
    buyItems.put("Bamboo Rod",  5000);
    buyItems.put("Hotdog Rod",  10000);
    ...
    public Shop(String mode) {
        ...
        if (mode.equals("trash")) {
            title = new JLabel("TRASH", SwingConstants.CENTER);
            title.setFont(new Font("Arial", Font.BOLD, 20));
            title.setForeground(Color.WHITE);
        } else {
            title = new JLabel(new ImageIcon(getClass().getResource("/images/storeHeader.png")));
        }
        ...
        sellAllButton.addActionListener(e -> { AudioPlayer.playSound("sellSound.wav"); sellAll(); refreshItems(); });
        ...
        if (mode.equals("trash")) {
            JButton trashItemsTab = makeTab("Items", 0, 175);
            setActive(trashItemsTab);
            tabPanel.add(trashItemsTab);
    ...
    private void refreshItems() {
        ...
        if (isBuyTab || shopMode.equals("buy")) {
            ...
            for (java.util.Map.Entry<String, Integer> entry : Inventory.items.entrySet()) {
                ...
                if (shopMode.equals("trash")) {
                    boolean isJunk = name.equals("Sandal") || name.equals("Shoe") || name.equals("Plastic Wrapper");
                    if (!isJunk) continue;
                }
                ...
                boolean isTrash = shopMode.equals("trash");
                JButton sellOne = new JButton(isTrash ? "Dispose" : "Sell");
                Buttons.addClickSound(sellOne);
                sellOne.addActionListener(e -> {
                    AudioPlayer.playSound("sellSound.wav");
    ...
    private void sellAll() {
        for (java.util.Map.Entry<String, Integer> entry :
            ...
            if (shopMode.equals("sell")) {
                ...
                if (isJunk) continue;
                if (isSellBaitTab  && !isBait) continue;
                if (!isSellBaitTab && !isFish) continue;
            }
